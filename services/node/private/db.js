...
var saveFile = './public/data/data.json'
...
exports.connect = function(){
	...
	var tempMac = [];
	var tempMacClean = [];
	pool.query('SELECT mac, name FROM sensor_location limit 20', (err, res) => {
		if (err) {
		...
		for(var i = 0; i < nameMac.length; i++) {
			...
			pool.query("select \"MAC\" as mac, timestamp as timestamp, ROUND(CAST(temperature as numeric), 1) as temp from envi where \"MAC\"='"+nameMac[i]["mac"]+"' order by id desc limit 1",(err, res,) => {
				if (err) {
					return log.error(err);
				}
				tempMac = res.rows
